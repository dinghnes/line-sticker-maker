...
` : '';
        ...
        if (mode === 'sticker') {
          return `✅ LINE 貼圖 12 格｜v3.2 Ultimate 旗艦 Prompt
...
--ar 37:24 --v 6.0`;
        ...
        return `✅ LINE 表情貼 12 格｜v3.2 Ultimate 旗艦 Prompt
...
--ar 4:3 --v 6.0`;
    ...
    const ImageSplitter = ({ mode, setMode }) => {
      ...
      // 👑 官方標準一鍵打包 ZIP (包含 main.png, tab.png, 01.png~12.png)
      const handleBatchDownload = async () => {
        ...
        try {
          ...
          const mainDataUrl = await createResizedCanvasData(targetCoverSticker.processedUrl, 240, 240);
          ...
          if (window.JSZip) {
            ...
            stickers.forEach((sticker, index) => {
              const fileName = useLineNaming 
                ? `${String(index + 1).padStart(2, '0')}.png`
                : `${filePrefix}_${formatNumber(startNumber, index)}.png`;
              folder.file(fileName, sticker.processedUrl.split(',')[1], { base64: true });
            });

            folder.file('main.png', mainDataUrl.split(',')[1], { base64: true });
            ...
            const link = document.createElement('a');
            link.href = url;
            link.download = `${packFolderName}_v3.2.zip`;
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            ...
            stickers.forEach((sticker, index) => {
              const fileName = useLineNaming 
                ? `${String(index + 1).padStart(2, '0')}.png`
                : `${filePrefix}_${formatNumber(startNumber, index)}.png`;
              files.push({ name: fileName, bytes: base64ToBytes(sticker.processedUrl) });
            });
            files.push({ name: 'main.png', bytes: base64ToBytes(mainDataUrl) });
            ...
            const link = document.createElement('a');
            link.href = url;
            link.download = `${useLineNaming ? 'line_sticker_package' : filePrefix + '_pack'}_v3.2.zip`;
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            setTimeout(() => URL.revokeObjectURL(url), 1000);
