...
` : '';
        ...
        if (mode === 'sticker') {
          return `✅ LINE 貼圖 12 格｜v3.3 Spec Corrected 旗艦 Prompt
...
--ar 37:24 --v 6.0`;
        ...
        return `✅ LINE 表情貼 12 格｜v3.3 Spec Corrected 旗艦 Prompt
...
--ar 4:3 --v 6.0`;
    ...
    const ImageSplitter = ({ mode, setMode }) => {
      ...
      const [useLineNaming, setUseLineNaming] = useState(true); // 01.png ~ 12.png
      ...
      const sliceImage = (img, currentConfig, fit = 'cover') => {
      ...
      const loadImageFile = (file) => {
        ...
        reader.onload = (event) => {
          ...
          img.onload = () => {
            ...
            setStickers(sliceImage(img, CONFIG, fitMode));
          };
      ...
      useEffect(() => {
        ...
        setStickers(sliceImage(originalImage, CONFIG, fitMode));
      }, [fitMode]);

      // 🛡️ 核心去背演算法：外圍泛洪防穿孔 (Flood Fill) + 平方距離極速過濾
      ...
      }, [enableSmartRemove, cropScale, targetColor, tolerance, smoothness, despill, protectWhite, useFloodFill, CONFIG]);
      ...
      // 📦 LINE 規格校正版 ZIP 打包
      // 一般貼圖：官方可選張數為 8 / 16 / 24 / 32 / 40；本工具目前工作網格為 12 張，因此 12 張包僅作工作包。
      // 一般表情貼：8～40 張皆可；檔名使用 001.png、002.png...，另附 tab.png。
      const handleBatchDownload = async () => {
        ...
        try {
          ...
          const targetCoverSticker = stickers[coverIndex] || stickers[0];
          ...
          if (!isEmoji && useLineNaming && stickers.length === 12) {
            alert('提醒：LINE 一般靜態貼圖官方送審張數只能選 8、16、24、32 或 40 張。目前是 12 張，因此本次會輸出「12 張工作包」，不可直接當成官方送審 ZIP。');
          }

          const packFolderName = isEmoji
            ? (useLineNaming ? 'line_emoji_package' : `${filePrefix}_pack`)
            : (useLineNaming ? 'line_sticker_workpack_12' : `${filePrefix}_pack`);

          const getOutputName = (index) => {
            if (!useLineNaming) return `${filePrefix}_${formatNumber(startNumber, index)}.png`;
            return isEmoji
          ...
          if (window.JSZip) {
            ...
            stickers.forEach((sticker, index) => {
              folder.file(getOutputName(index), sticker.processedUrl.split(',')[1], { base64: true });
            });
            ...
            const link = document.createElement('a');
            link.href = url;
            link.download = `${packFolderName}_v3.3.zip`;
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            ...
            const files = [];
            stickers.forEach((sticker, index) => {
              files.push({ name: getOutputName(index), bytes: base64ToBytes(sticker.processedUrl) });
            });
            ...
            <div className="bg-[#151926] rounded-2xl shadow-xl p-6 border border-slate-800/60 flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <div className="flex items-center gap-2.5">
                    <h1 className="text-2xl font-black text-white tracking-tight">
                      Sticker Splitter <span className={mode === 'emoji' ? 'text-orange-400' : 'text-indigo-400'}>Pro Max</span>
                    </h1>
                  ...
                  <p className="text-slate-400 text-xs mt-1 flex items-center gap-1.5">
                    ...
                    外圍泛洪防穿孔去背 • 自動產生官方 main / tab 審核包 • 4色底色檢查 • 燈箱放大
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                <div className="bg-[#0f1117] p-1.5 rounded-xl border border-slate-700/80 flex">
                  <button
                ...
                {stickers.length > 0 && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3.5 py-2 text-slate-300 bg-[#1e2336] hover:bg-[#252b42] border border-slate-700 rounded-xl text-xs font-medium transition"
                  >
            ...
            {stickers.length > 0 ? (
              <div className="grid lg:grid-cols-12 gap-6">
                      ...
                      <div className="p-5 space-y-5 text-xs">
                        ...
                        <div className="bg-[#0f1117] p-3.5 rounded-xl border border-slate-800 space-y-2">
                          <div className="flex justify-between items-center">
                            <span className="text-slate-400 font-medium flex items-center gap-1">
                              <Layers size={13} className="text-indigo-400" /> 圖片適配防止變形
                            </span>
                            <span className="text-[10px] text-emerald-400 font-mono">推薦等比填滿</span>
                          </div>
                          <select
                            value={fitMode}
                          ...
                          >
                            <option value="cover">等比例填滿 Cover（最推薦，角色完全不變形）</option>
                            <option value="contain">完整顯示 Contain（可能四周留邊）</option>
                            <option value="stretch">強制拉伸 Stretch（舊版模式）</option>
                          </select>
                          {sourceInfo && (
                            <p className={`text-[10px] leading-relaxed ${sourceInfo.ratioDiff > 0.03 ? 'text-amber-400' : 'text-emerald-400'}`}>
                              原圖 {sourceInfo.width}×{sourceInfo.height}px
                              {sourceInfo.ratioDiff > 0.03 ? '（比例與標準略有差異，已透過中心等比縮放修復）' : '（標準比例，完美適配）'}
                            </p>
                          )}
                        </div>
                        ...
                        <div className="bg-indigo-950/30 border border-indigo-500/30 p-3.5 rounded-xl space-y-2">
                          ...
                          <p className="text-[10px] text-slate-400 leading-relaxed">
                            只從四個外圍邊緣蔓延去背，遇到白色輪廓自動停止。角色身上的綠眼睛、綠飾品與衣服<strong>100% 不會被挖空破洞</strong>！
                          </p>
                        </div>
                  ...
                  <div className="bg-[#151926] rounded-2xl shadow-xl border border-slate-800/80 overflow-hidden p-5 space-y-4 text-xs">
                    ...
                    <div className="space-y-3">
                      <label className="flex items-center justify-between p-3 rounded-xl bg-[#0f1117] border border-slate-800 cursor-pointer">
                        <div className="flex flex-col">
                          <span className="font-bold text-slate-200">採用 LINE 官方標準命名 (01.png~12.png)</span>
                          <span className="text-[10px] text-slate-500">審核規定檔名，解壓縮可直接上傳</span>
                        </div>
                        <input
                          type="checkbox"
                      ...
                      <div className="bg-[#0f1117] p-3 rounded-xl border border-slate-800 space-y-1 text-[11px] text-slate-400">
                        <div className="text-slate-300 font-semibold flex items-center gap-1">
                          ...
                          縮圖指定（目前：第 {coverIndex + 1} 格）：
                        </div>
                        <p className="text-[10px] text-slate-500">
                          點擊任意貼圖右上角的「★ 設為封面」，系統會依目前模式產出所需縮圖；一般貼圖另產 main.png，表情貼不需要 main.png：
                        </p>
                        <ul className="list-disc list-inside text-[10px] text-emerald-400/90 font-mono pt-1">
                          <li>一般貼圖：main.png (240 × 240) + tab.png (96 × 74)</li>
                          <li>表情貼：tab.png (96 × 74)，檔名為 001.png 起</li>
                        </ul>
                      </div>
                    </div>

                    {mode === 'sticker' && stickers.length === 12 && (
                      <div className="text-[10px] leading-relaxed text-amber-300 bg-amber-500/10 border border-amber-500/20 rounded-lg p-2">
                        ⚠️ 官方一般貼圖張數需為 8／16／24／32／40 張；目前 12 張為工作包，不能直接送審。
                      </div>
                    )}
                    {mode === 'emoji' && (
                      <div className="text-[10px] leading-relaxed text-emerald-300 bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-2">
                        ✓ 一般表情貼 12 張符合 8～40 張規格；官方命名將使用 001.png～012.png。
                      </div>
                    )}
                ...
                <div className="lg:col-span-8 space-y-4">
                  ...
                  <div className="bg-[#151926] p-3 rounded-2xl border border-slate-800/80 flex flex-wrap items-center justify-between gap-3 text-xs">
                    <div className="flex items-center gap-2">
                      <span className="text-slate-400 font-medium">預覽底色檢查：</span>
                    ...
                    <div className="text-[11px] text-slate-400 flex items-center gap-2">
                      <span>規格：</span>
                      <span className="font-mono text-emerald-400 font-bold">{CONFIG.cellW}×{CONFIG.cellH}px</span>
                      <span className="opacity-30">|</span>
                      <span>共 {stickers.length} 張</span>
                    </div>
                  </div>
                      ...
                      return (
                        <div
                          ...
                          className={`group relative bg-[#151926] rounded-2xl shadow-lg border overflow-hidden transition-all duration-300 ${
                            isCover ? 'border-amber-400 shadow-amber-400/20' : 'border-slate-800 hover:border-indigo-500/50'
                          }`}
                        >
                          {/* 頂部標籤列 */}
                          ...
                          {/* 底部操作 */}
                          <div className="p-2.5 bg-[#151926] border-t border-slate-800 flex justify-between items-center gap-2">
                            <button
                              onClick={() => setCoverIndex(index)}
                              className={`text-[10px] font-semibold transition px-2 py-1 rounded-lg ${
                                isCover ? 'text-amber-400 bg-amber-400/10' : 'text-slate-500 hover:text-slate-300'
                              }`}
                            >
                              {isCover ? '已是封面' : '設為封面'}
                            </button>
                            <button
                              ...
                              className="flex items-center gap-1 px-3 py-1 bg-slate-800 hover:bg-indigo-600 text-slate-300 hover:text-white rounded-lg text-[10px] font-medium transition"
                            >
                              <Download size={11} /> 單張下載
                            </button>
                          </div>
              ...
              >
                ...
                <p className="text-slate-400 max-w-md mx-auto mb-8 text-sm leading-relaxed">
                  ...
                  <br />
                  單格自動裁切為 <span className="text-indigo-400 font-bold">{CONFIG.cellW} × {CONFIG.cellH} px</span>（等比填滿、防角色變形）
                </p>
